...
class SnakeAI:
     ...
     def play_step(self,action):
         ...
         distance_before=self.manhatan_distance(self.food,self.snake)
         self._move(action)
         ...
         if (self.snake==self.food):
             ...
             reward=10
             self._place_food()
             if (len(self.obsticles)*10<self.score and len(self.obsticles)<10):
                 self._place_obsticle()
         else:
            distance_after=self.manhatan_distance(self.food,self.snake)
            if (distance_after>distance_before):
                ...
                reward=0
            reward=reward*self.reward_decay_weight
            self.body.pop()
     ...
     def _place_food(self):
        ...
        self.food=(x,y)
        if (self.food in self.body or self.food in self.obsticles):
            self._place_food()
     def _place_obsticle(self):
        x=random.randint(0,(self.width-Block)//Block)*Block
        y=random.randint(0,(self.height-Block)//Block)*Block
        self.obsticle=(x,y)
        if (self.obsticle in self.body or self.obsticle in self.obsticles or self.obsticle[0]>self.width-Block*2 or self.obsticle[0]<Block or self.obsticle[1]>self.height-Block*2 or self.obsticle[1]<Block):
            self._place_obsticle()
        else:
            self.obsticles.append(self.obsticle)
     def updateUI(self):
        ...
        for obsticle in self.obsticles:
            pygame.draw.rect(self.game_window,purple,pygame.Rect(obsticle[0],obsticle[1],Block,Block))
